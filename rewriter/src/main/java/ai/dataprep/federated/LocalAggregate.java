...
public class LocalAggregate extends EnumerableAggregateBase implements EnumerableRel {
    public LocalAggregate(
        ...
        for (AggregateCall aggCall : aggCalls) {
            // should support distinct locally
...
//            }
    ...
    @Override public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
        ...
        // since PhysType of(final JavaTypeFactory typeFactory, Type javaRowClass) is not public
        final RelDataTypeFactory.Builder builderTmp = typeFactory.builder();
        Type javaRowClass = typeFactory.createSyntheticType(aggStateTypes);
        if (javaRowClass instanceof Types.RecordType) {
            ...
            for (Types.RecordField field : recordType.getRecordFields()) {
                builderTmp.add(field.getName(), typeFactory.createType(field.getType()));
            }
        }
        RelDataType rowType = builderTmp.build();
        final PhysType accPhysType =
                PhysTypeImpl.of(typeFactory, rowType, JavaRowFormat.CUSTOM, false);
        // since PhysType of(final JavaTypeFactory typeFactory, Type javaRowClass) is not public
